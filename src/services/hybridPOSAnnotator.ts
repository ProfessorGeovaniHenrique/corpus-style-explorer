...
import type { POSToken, MorphFeatures } from '@/data/types/pos-annotation.types';
import { 
  irregularVerbs, 
  conjugatedToInfinitive,
  auxiliaryVerbs 
} from '@/data/grammatical-knowledge/verbal-morphology';
import {
  personalPronouns,
  possessivePronouns,
...
} from '@/data/grammatical-knowledge/pronoun-system';
import { detectGauchoMWEs } from '@/data/grammatical-knowledge/gaucho-mwe';
import { 
  getCachedPOSAnnotation, 
  setCachedPOSAnnotation,
  createCacheKey 
} from './posAnnotationCache';
...
/**
 * Interface para resultado de anotação com source tracking
 */
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    annotated.push({
        ...
        features: {
          ...
          numero: cached.features.numero,
          pessoa: cached.features.pessoa,
          genero: cached.features.genero,
          modo: cached.features.modo,
          grau: cached.features.grau,
        } as MorphFeatures,
        posicao: i,
        source: 'cache',
        confidence: 1.0,
    ...
    if (annotation.confidence >= 0.95) {
      setCachedPOSAnnotation(palavra, {
        ...
        features: {
          tempo: annotation.features.tempo,
          numero: annotation.features.numero,
          pessoa: annotation.features.pessoa,
          genero: annotation.features.genero,
          modo: annotation.features.modo,
          grau: annotation.features.grau,
        },
...
function annotateTokenWithGrammar(
  ...
  // 8. Palavra desconhecida - retorna null (para Layer 2/3 processar)
  return {
    palavra,
    lema: lower,
    pos: 'UNKNOWN',
...
/**
 * Infere features morfológicas de verbos
 */
function inferVerbFeatures(forma: string, infinitivo: string): MorphFeatures {
  const features: MorphFeatures = {};

  // Lógica simplificada (pode expandir com regras mais sofisticadas)
...
/**
 * Infere features de determinantes
 */
